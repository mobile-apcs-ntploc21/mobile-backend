...
const createChannelUserPermission = async (
  ...
  try {
    ...
    if (
      await ChannelUserPermission.exists({
        "_id.user_id": user_id,
...
const channelUserPermissionAPI: IResolvers = {
  Query: {
    ...
    getChannelUsersPermissions: async (_, { channel_id }) => {
      try {
        ...
        throw new UserInputError(
          "Cannot get channel permissions associated with this user!"
        );
      }
    },
  ...
  Mutation: {
    createChannelUserPermission: async (
      _,
      { user_id, channel_id, permissions }
    ) => {
      try {
        const channel_user_permission = await createChannelUserPermission(
          user_id,
          channel_id,
          permissions
        );
    ...
    updateChannelUserPermission: async (
      _,
      { user_id, channel_id, permissions }
    ) => {
      try {
        ...
        const channel_user_permission =
          await ChannelUserPermission.findOneAndUpdate(
            {
              "_id.user_id": user_id,
              "_id.channel_id": channel_id,
            },
    ...
    deleteChannelUserPermission: async (_, { user_id, channel_id }) => {
      try {
        const channel_user_permission =
          await ChannelUserPermission.findOneAndDelete({
            "_id.user_id": user_id,
            "_id.channel_id": channel_id,
          });
