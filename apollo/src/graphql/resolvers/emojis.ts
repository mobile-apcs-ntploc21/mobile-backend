...
const syncUnicodeEmoji = async (): Promise<void> => {
  ...
  try {
    ...
    await EmojiModel.insertMany(emojiObjects, opts);
    /* await Promise.all(
...
// Read the emojis_ordering.txt file to get the order of the emojis
const parseEmojisOrdering = (filePath: string) => {
  const ordering: { [key: string]: number } = {};
  const lines = fs.readFileSync(filePath, "utf-8").split("\n");
  lines.forEach((line, index) => {
    if (line.trim() && !line.startsWith("#")) {
      const unicodeHex = line.split(";")[0].trim();
      const unicodeChar = String.fromCodePoint(
        parseInt(unicodeHex.replace("U+", ""), 16)
      );
      ordering[unicodeChar] = index;
    }
  });
  return ordering;
};

const emojisOrdering = parseEmojisOrdering(
  "./src/constants/emoji_ordering.txt"
);
  ...
  Query: {
    ...
    serversEmojis: async (_, { user_id }) => {
      ...
      for (const server of servers) {
        ...
        emojiGroups.push({
          name: serverObj.name,
          server_id: serverObj._id.toString(),
    ...
    unicodeEmojis: async (_, { confirm }) => {
      if (confirm) {
        ...
        // Sort the emojis by the appointed list
        emojiGroups.forEach((group) => {
          group.emojis.sort((a, b) => {
            const orderA = emojisOrdering[a.unicode] ?? Infinity;
            const orderB = emojisOrdering[b.unicode] ?? Infinity;
            return orderA - orderB;
          });
        });

        // Debug slice the emojiGroups
        emojiGroups.forEach((group) => {
          for (let i = 0; i < 10; i++) {
            const order = emojisOrdering[group.emojis[i].unicode] ?? Infinity;
            console.log(group.name, group.emojis[i].name, order);
          }
        });
